...
def star_coverage_rate(bullets: Iterable[str]) -> float:
    ...
    if not bullets:
        return 0.0
    action_words = {"built", "developed", "implemented", "analyzed", "created", "improved", "assisted", "contributed"}
    result_words = {"result", "achieved", "improved", "reduced", "increased", "enabled", "supporting", "helped"}
    covered = 0
    for bullet in bullets:
        words = {normalize_token(word) for word in bullet.split()}
        has_action = bool(words & action_words)
        has_result = any(word in (bullet or "").lower() for word in result_words)
