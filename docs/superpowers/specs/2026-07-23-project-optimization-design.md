...
仓库所有者在 2026-07-23 设定的约束：DeepSeek 账户余额跌至 **2.00 元人民币**
或以下时，停止一切真实 API 验证，改用确定性 fallback 路径验证，并在
`docs/OPTIMIZATION_LOG.md` 中明确标注该步骤未经真实模型确认。

执行方式不依赖人为记忆，而是由 `tools/check_deepseek_budget.py` 强制：

- 每次真实 API 验证**之前**运行一次，退出码非 0 则不得发起调用。
- 每次真实 API 验证**之后**再运行一次，把余额差额记入优化日志。
- 退出码 2（余额读取失败）同样禁止调用 —— 不在预算未知的情况下继续。

禁止"重跑到成功为止"的循环。单个 slice 的真实验证最多重试一次；仍失败则
记录失败输出并转为离线验证。

设定时的起始余额为 6.01 元，即本轮优化最多可消耗约 4 元。
